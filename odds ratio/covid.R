...
library(ggplot2)
library(reshape)
x <- read.csv("/home/stark/Downloads/Dataset - Sheet8 (2).csv")
data <- as.data.frame(x)
colnames(data)[6] <- "Lady's_finger_price"
fit_glm <- glm(data$Y ~ data$Onion_price + data$Tomato_price + data$Potato.Price + data$Cabbage_price + data$`Lady's_finger_price` + data$Cauliflower_price + data$Brinjal_Price, data = data, family = "binomial") 
...
col <- c("Onion","Tomato","Potato","Cabbage","Lady's finger","Cauliflower","Brinjal")
